...
int qh_rboxpoints(qhT *qh, char *rbox_command)
{
  ...
  *command = '\0';
  ...
  if ((issimplex + islens + isspiral + ismesh > 1) || (issimplex + issphere + isspiral + ismesh > 1))
  {
  ...
  else if (israndom)
  {
    ...
    snprintf(seedbuf, sizeof(seedbuf), " t%d", seed); /* appends an extra t, not worth removing */
