  ...
  while (*s)
  {
    ...
    else if (isupper(key))
    {
      ...
      while (!chkerr && *s && !isspace(*s))
      {
        ...
        else if (key == 'Q' && isdigit(opt) && prevopt != 'b' && (prevopt == ' ' || islower(prevopt)))
        {
          chkopt[2] = opt;
          if (strstr(hiddenflags, chkopt))
            chkerr = chkopt;
...
void qh_clear_outputflags(qhT *qh)
{
  ...
  { /* duplicated in qh_initqhull_buffers and qh_clear_outputflags */
...
void qh_freebuffers(qhT *qh)
{
  ...
  if (qh->input_points && qh->input_malloc)
        ...
        case 'B': /* handled by qh_initthresholds */
          ...
          if (*s == ':')
          {
            ...
            if ((r = qh_strtod(s, &s)) == 0.0)
            {
              ...
              while (*t && !isspace(*t))
              {
                if (toupper(*t++) == 'B' && k == qh_strtol(t, &t) && *t++ == ':' && qh_strtod(t, &t) == 0.0)
                {
                  qh->PROJECTinput++;
...
void qh_initqhull_globals(qhT *qh, coordT *points, int numpoints, int dim, boolT ismalloc)
{
  ...
  if (qh->MERGING && !qh->POSTmerge && qh->premerge_cos > REALmax / 2 && qh->premerge_centrum == 0)
  {
    qh->ZEROcentrum = True;
    qh->ZEROall_ok = True;
  ...
  qh->KEEPnearinside = (qh->DOcheckmax && !(qh->KEEPinside && qh->KEEPcoplanar) && !qh->NOnearinside);
  if (qh->MERGING)
...
qhull configuration error (qh_RANDOMmax in user.h):\n\
  ...
  qh_RANDOMseed_(qh, seed);
  randr = randr / 1000;
  if (randr < qh_RANDOMmax * 0.1 || randr > qh_RANDOMmax * 0.9)
...
void qh_initqhull_outputflags(qhT *qh)
{
  ...
  {
    if (qh->KEEParea || qh->KEEPminArea < REALmax / 2 || qh->KEEPmerge || qh->DELAUNAY || (!qh->ONLYgood && (qh->GOODvertex || qh->GOODpoint)))
    {
      qh->PRINTgood = True;
  ...
  if (printgeom)
  {
    ...
    if (qh->PRINTnoplanes && !(qh->PRINTcoplanar + qh->PRINTcentrums + qh->PRINTdots + qh->PRINTspheres + qh->DOintersections + qh->PRINTridges))
    {
...
void qh_lib_check(int qhullLibraryType, int qhTsize, int vertexTsize, int ridgeTsize, int facetTsize, int setTsize, int qhmemTsize)
{
  ...
  if (qhullLibraryType == QHULL_NON_REENTRANT)
  { /* 0 */
  ...
  else if (qhullLibraryType == QHULL_QH_POINTER)
  { /* 1 */
  ...
  else if (qhullLibraryType != QHULL_REENTRANT)
  { /* 2 */
...
void qh_option(qhT *qh, const char *option, int *i, realT *r)
{
  ...
  snprintf(buf, buf_size, "  %s", option);
  if (i)
    snprintf(buf + strlen(buf), buf_size + strlen(buf), " %d", *i);
  if (r)
    snprintf(buf + strlen(buf), buf_size + strlen(buf), " %2.2g", *r);
